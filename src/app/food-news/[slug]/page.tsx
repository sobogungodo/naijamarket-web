// src/app/food-news/[slug]/page.tsx
// Route: /food-news/[slug]  (top-level)
...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      url:           `https://naijamarketintel.ng/food-news/${article.slug}`,
    },
  }
}
...
export default async function FoodNewsArticlePage({ params }: { params: { slug: string } }) {
      ...
      <div className="bg-white dark:bg-[#0d0d0d] border-b border-gray-100 dark:border-[#1a1a1a]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <nav className="flex items-center gap-2 text-sm font-sans text-gray-500 dark:text-gray-400">
            ...
            <span>/</span>
            <Link href="/food-news" className="hover:text-[#1A6B37] dark:hover:text-[#4ade80] transition-colors">Food News</Link>
            <span>/</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="bg-white dark:bg-[#141414] rounded-2xl border border-gray-100 dark:border-[#2a2a2a] p-8">
              ...
              <div className="pt-6 border-t border-gray-100 dark:border-[#2a2a2a]">
                ...
                <p className="text-xs text-gray-400 dark:text-gray-500 font-sans mt-2">
                  Summary by NaijaMarket Intel research team · Original article by {article.source_name}
                </p>
              </div>
            </div>

            <div className="mt-6">
              <Link href="/food-news" className="inline-flex items-center gap-2 text-[#1A6B37] dark:text-[#4ade80] font-sans font-semibold text-sm hover:underline">
                ← Back to Food News
              </Link>
            </div>
          </article>
          ...
          <aside className="space-y-6">
            ...
            <div className="bg-[#0f2419] dark:bg-[#0d1f15] rounded-2xl p-6 border border-[#1a3d25]">
              <p className="text-xs font-semibold text-[#D4A017] uppercase tracking-widest font-sans mb-2">Live Intelligence</p>
              <h3 className="text-base font-bold text-white font-sans mb-2">See Current Market Prices</h3>
              <p className="text-gray-400 text-sm font-sans mb-4">
                Real-time prices from verified traders across 226 Nigerian markets.
              </p>
              <Link href="/prices" className="block w-full text-center bg-[#1A6B37] text-white text-sm font-semibold font-sans py-2.5 rounded-xl hover:bg-[#155c2e] transition-colors">
                View Live Prices →
              </Link>
            </div>
            ...
            {recent.length > 0 && (
              <div>
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h3 className="text-sm font-bold text-gray-900 dark:text-white font-sans">Recent Reports</h3>
                </div>
                <div className="space-y-3">
                  {recent.map(r => (
                    <Link key={r.slug} href={`/food-news/${r.slug}`} className="group block">
                      <div className="bg-white dark:bg-[#141414] rounded-xl p-4 border border-gray-100 dark:border-[#2a2a2a] hover:border-[#1A6B37] dark:hover:border-[#2d8a50] transition-all">
                        ...
                        <p className="text-xs text-gray-400 dark:text-gray-500 font-sans mt-1">{formatDate(r.published_date)}</p>
                      </div>
                    </Link>
                  ))}
                </div>
            ...
            <div className="bg-[#1a6b37]/10 dark:bg-[#1a6b37]/5 rounded-2xl p-6 border border-[#1a6b37]/20">
              <p className="text-xs font-semibold text-[#1A6B37] dark:text-[#4ade80] uppercase tracking-widest font-sans mb-2">Free on WhatsApp</p>
              <p className="text-sm text-gray-700 dark:text-gray-300 font-sans mb-4">
                Get live market prices, alerts and weekly briefings directly on WhatsApp.
              </p>
              <a href="https://wa.me/message/NAIJAMARKET" target="_blank" rel="noopener noreferrer"
