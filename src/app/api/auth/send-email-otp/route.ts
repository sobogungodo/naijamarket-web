// src/app/api/auth/send-email-otp/route.ts
// NaijaMarket Intel - Send Email Verification Code
// UPDATED: 2026-02-25 — Switched from Brevo to Resend
//
// ENV VARS REQUIRED:
//   RESEND_API_KEY → from resend.com/api-keys
//
// SENDER: noreply@naijamarketintel.ng

import { NextRequest, NextResponse } from "next/server";
import { Resend } from "resend";
import { prisma } from "@/lib/prisma";

// ============================================================================
// RESEND CONFIG
// ============================================================================

const resend = new Resend(process.env.RESEND_API_KEY);

const SENDER      = "NaijaMarket Intel <noreply@naijamarketintel.ng>";
const SUPPORT_EMAIL = "support@naijamarketintel.ng";

// ============================================================================
// HELPERS
// ============================================================================

function generateOTP(): string {
  return Math.floor(100000 + Math.random() * 900000).toString();
}

function normalizeEmail(email: string): string {
  return email.toLowerCase().trim();
}

// ============================================================================
// EMAIL TEMPLATE
// ============================================================================

function getEmailTemplate(code: string, purpose: string): string {
  const purposeText =
    purpose === "registration"  ? "complete your registration" :
    purpose === "login"         ? "log in to your account"     :
    purpose === "password_reset"? "reset your password"        :
                                  "verify your email";

  const subjectPrefix =
    purpose === "registration"  ? "Complete Your Registration" :
    purpose === "login"         ? "Your Login Code"            :
    purpose === "password_reset"? "Reset Your Password"        :
                                  "Verify Your Email";

  return `
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>NaijaMarket Intel - ${subjectPrefix}</title>
</head>
<body style="margin:0;padding:0;background-color:#0a0a0a;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0"
    style="max-width:600px;margin:0 auto;padding:40px 20px;">

    <!-- LOGO -->
    <tr>
      <td style="text-align:center;padding-bottom:30px;">
        <h1 style="margin:0;font-size:28px;">
          <span style="color:#ffffff;">Naija</span>
          <span style="color:#00A36C;">Market</span>
          <span style="color:#FFD700;"> Intel</span>
        </h1>
        <p style="color:#666;font-size:12px;margin-top:8px;font-family:monospace;">
          The Bloomberg of African Commodities
        </p>
      </td>
    </tr>

    <!-- CARD -->
    <tr>
      <td style="background-color:#141414;border:1px solid #2a2a2a;border-radius:12px;padding:40px;">

        <h2 style="color:#ffffff;margin:0 0 12px 0;font-size:20px;">
          Verification Code
        </h2>

        <p style="color:#999;margin:0 0 30px 0;line-height:1.6;">
          Use this code to ${purposeText}:
        </p>

        <!-- OTP BOX -->
        <div style="background-color:#0a0a0a;border:2px solid #00A36C;border-radius:8px;
                    padding:24px 20px;text-align:center;margin-bottom:30px;">
          <span style="font-size:40px;font-weight:bold;letter-spacing:10px;
                       color:#00A36C;font-family:monospace;">
            ${code}
          </span>
        </div>

        <p style="color:#666;font-size:14px;margin:0 0 12px 0;">
          ⏱ This code expires in <strong style="color:#FFD700;">10 minutes</strong>.
        </p>

        <p style="color:#666;font-size:14px;margin:0 0 24px 0;">
          If you didn't request this code, you can safely ignore this email.
          Your account is secure.
        </p>

        <hr style="border:none;border-top:1px solid #2a2a2a;margin:24px 0;">

        <p style="color:#555;font-size:12px;margin:0;">
          Need help? Contact us at
          <a href="mailto:${SUPPORT_EMAIL}"
             style="color:#00A36C;text-decoration:none;">${SUPPORT_EMAIL}</a>
        </p>
      </td>
    </tr>

    <!-- FOOTER -->
    <tr>
      <td style="text-align:center;padding-top:30px;">
        <p style="color:#444;font-size:12px;margin:0;">
          © 2026 NaijaMarket Intel • Giggababytes Oy
        </p>
        <p style="color:#333;font-size:11px;margin-top:6px;">
          Real-time commodity price intelligence for Nigerian markets
        </p>
      </td>
    </tr>

  </table>
</body>
</html>
  `;
}

// ============================================================================
// POST - Send Email OTP
// ============================================================================

export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { email, purpose = "registration" } = body;

    // ── Validate input ────────────────────────────────────────────────────────
    if (!email) {
      return NextResponse.json(
        { error: "Email is required" },
        { status: 400 }
      );
    }

    const normalizedEmail = normalizeEmail(email);

    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(normalizedEmail)) {
      return NextResponse.json(
        { error: "Invalid email format" },
        { status: 400 }
      );
    }

    const validPurposes = ["registration", "login", "password_reset"];
    if (!validPurposes.includes(purpose)) {
      return NextResponse.json(
        { error: "Invalid purpose" },
        { status: 400 }
      );
    }

    console.log("[SEND-EMAIL-OTP] ═══════════════════════════════════════════");
    console.log("[SEND-EMAIL-OTP] Email:", normalizedEmail, "| Purpose:", purpose);

    // ── Rate limiting: max 3 requests per 5 minutes ───────────────────────────
    const recentCodes = await prisma.$queryRaw`
      SELECT COUNT(*) as count
      FROM Email_Verification_Codes
      WHERE email      = ${normalizedEmail}
        AND created_at > DATEADD(MINUTE, -5, GETDATE())
    ` as any[];

    const requestCount = Number(recentCodes?.[0]?.count ?? 0);
    if (requestCount >= 3) {
      console.log("[SEND-EMAIL-OTP] ⚠️ Rate limit hit for:", normalizedEmail);
      return NextResponse.json(
        { error: "Too many requests. Please wait 5 minutes before trying again." },
        { status: 429 }
      );
    }

    // ── Generate OTP ──────────────────────────────────────────────────────────
    const code      = generateOTP();
    const expiresAt = new Date(Date.now() + 10 * 60 * 1000).toISOString(); // 10 min

    const ipAddress = request.headers.get("x-forwarded-for") ?? "unknown";
    const userAgent = request.headers.get("user-agent")      ?? "unknown";

    // ── Store OTP in DB first ─────────────────────────────────────────────────
    await prisma.$executeRaw`
      INSERT INTO Email_Verification_Codes
        (email, code, purpose, expires_at, ip_address, user_agent)
      VALUES
        (${normalizedEmail}, ${code}, ${purpose}, ${expiresAt}, ${ipAddress}, ${userAgent})
    `;

    console.log("[SEND-EMAIL-OTP] OTP stored in DB");

    // ── Send via Resend ───────────────────────────────────────────────────────
    const subjectMap: Record<string, string> = {
      registration:   `Your NaijaMarket Intel Verification Code: ${code}`,
      login:          `Your NaijaMarket Intel Login Code: ${code}`,
      password_reset: `Reset Your NaijaMarket Intel Password: ${code}`,
    };

    const { data, error: resendError } = await resend.emails.send({
      from:    SENDER,
      to:      [normalizedEmail],
      subject: subjectMap[purpose] ?? `Your NaijaMarket Intel Code: ${code}`,
      html:    getEmailTemplate(code, purpose),
    });

    if (resendError) {
      console.error("[SEND-EMAIL-OTP] ❌ Resend error:", resendError);

      // Dev fallback — return code directly so you can test
      if (process.env.NODE_ENV === "development") {
        console.log("[SEND-EMAIL-OTP] 🛠 Dev mode code:", code);
        return NextResponse.json({
          success:  true,
          message:  "Dev mode: email delivery failed but code generated",
          dev_code: code, // ⚠️ development ONLY
        });
      }

      return NextResponse.json(
        { error: "Failed to send verification email. Please try again." },
        { status: 500 }
      );
    }

    console.log("[SEND-EMAIL-OTP] ✅ Sent via Resend | ID:", data?.id);

    return NextResponse.json({
      success: true,
      message: "Verification code sent to your email",
    });

  } catch (error: any) {
    console.error("[SEND-EMAIL-OTP] ❌ Unhandled error:", error);
    return NextResponse.json(
      { error: error.message || "Failed to send verification code" },
      { status: 500 }
    );
  }
}
