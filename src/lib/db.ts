...
import { wrapPrismaForSupabase, USE_SUPABASE } from './supabase-prisma-proxy';
...
// =============================================================================
// The consumer web reaches the DB largely through Prisma's RAW methods ($queryRaw /
// $queryRawUnsafe / $executeRaw / $executeRawUnsafe). wrapPrismaForSupabase() routes those
// to the Supabase pg pool (translateTSQL applied) when DB_BACKEND=supabase; production leaves
// DB_BACKEND unset → the real Prisma/Azure client is returned untouched. Shared with
// @/lib/prisma so both shared clients behave identically. int8 is parsed to Number (see
// db-supabase.ts) so COUNT(*) arithmetic in routes keeps working.
export const prisma: PrismaClient = wrapPrismaForSupabase(realPrisma);

// Exposed so routes can branch when their T-SQL can't be surface-translated (DECLARE @var
// batches, DDL) and needs a hand-written PG path or a no-op on the Supabase Dev backend.
